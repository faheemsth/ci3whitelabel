...
class Balancefeesreport extends Admin_Controller{	
		...
		if(permissionChecker('balancefeesreport')) {
			if($_POST) {
			    ...
			    if ($this->form_validation->run() == FALSE) {
					...
					$this->data['invoice_test'] = $this->invoice_m->get_all_balancefees_for_report($this->input->post());
					...
					$this->data['totalPayment'] = $this->totalPaymentAndWeaver($this->payment_m->get_orders_by_payment(array('schoolyearID' => $schoolyearID)));
	...
	private function totalAmountAndDiscustomCompute($arrays) {
		...
		if(customCompute($arrays)) {
			foreach($arrays as $key => $array) {
				...
				if (isset($totalAmountAndDiscount[$array->studentID]['type_v'])) {
					if ($totalAmountAndDiscount[$array->studentID]['type_v'] == 'other_charges') {
						$other_charges = ($array->amount);
						$totalAmountAndDiscount[$array->studentID]['other_charges'] += $other_charges;
						$totalAmountAndDiscount[$array->studentID]['feetype'] = $array->feetype;
					}
				}else{
					if ($array->type_v == 'other_charges') {
						$other_charges = ($array->amount);
						$totalAmountAndDiscount[$array->studentID]['other_charges'] = $other_charges;
						$totalAmountAndDiscount[$array->studentID]['feetype'] = $array->feetype;
						$totalAmountAndDiscount[$array->studentID]['type_v'] = $array->type_v;
					}
				}

				if (isset($totalAmountAndDiscount[$array->studentID]['type_vs'])) {
					if($totalAmountAndDiscount[$array->studentID]['type_vs'] == 'invoice'){
						$invoice_amount = ($array->amount);
						$totalAmountAndDiscount[$array->studentID]['invoice_amount'] += $invoice_amount;
						$totalAmountAndDiscount[$array->studentID]['feetype'] = $array->feetype;
					}
				}else{
					if ($array->type_v == 'invoice') {
						$invoice_amount = ($array->amount);
						// $totalAmountAndDiscount[$array->studentID]['other_charges'] = $array->type_v;
						$totalAmountAndDiscount[$array->studentID]['invoice_amount'] = $invoice_amount;
						$totalAmountAndDiscount[$array->studentID]['feetype'] = $array->feetype;
						$totalAmountAndDiscount[$array->studentID]['type_vs'] = $array->type_v;
					}
				}
			}
		}
	...
	private function generateXML($data) {
		...
		if(customCompute($students)) {
	        ...
	        $headers['invoice_type'] = $this->lang->line('balancefeesreport_invoice_type');
	        $headers['previous_balance'] = $this->lang->line('balancefeesreport_prev_balance');
	        $headers['other_charges'] = $this->lang->line('balancefeesreport_other_charges');
	        $headers['invoice_ammount'] = $this->lang->line('balancefeesreport_invoice_ammount');
            ...
            $totalBalance = 0;
	      	...
	      	foreach ($students as $student) {
                ...
                if($sectionID == 0) { 
	      		...
	      		$studentArray[$i]['invoice_type'] = isset($totalAmountAndDiscount[$student->srstudentID]['feetype']) ? $totalAmountAndDiscount[$student->srstudentID]['feetype'] : 'None';

	      		$studentArray[$i]['previous_balance'] = $student->balance;
	      		$studentArray[$i]['other_charges'] = isset($totalAmountAndDiscount[$student->srstudentID]['other_charges']) ? ceil($totalAmountAndDiscount[$student->srstudentID]['other_charges']) : 0;
	      		$studentArray[$i]['invoice_amount'] = isset($totalAmountAndDiscount[$student->srstudentID]['invoice_amount']) ? ceil($totalAmountAndDiscount[$student->srstudentID]['invoice_amount']) : 0;

	      		$studentArray[$i]['amount'] = isset($totalAmountAndDiscount[$student->srstudentID]['amount']) ? ceil($totalAmountAndDiscount[$student->srstudentID]['amount']) : '0';
	      		$studentArray[$i]['discount'] = isset($totalAmountAndDiscount[$student->srstudentID]['discount']) ? ceil($totalAmountAndDiscount[$student->srstudentID]['discount']) : '0';
	      		$studentArray[$i]['payment'] = isset($totalPayment[$student->srstudentID]['payment']) ? ceil($totalPayment[$student->srstudentID]['payment']) : '0';
	      		$studentArray[$i]['weaver'] = isset($totalweavar[$student->srstudentID]['weaver']) ? ceil($totalweavar[$student->srstudentID]['weaver']) : '0';
                ...
                $studentArray[$i]['balance'] = ceil($Balance);
	      	}
	     	...
	     	if($sectionID == 0) { 
	      	...
	      	$studentArray[$i]['amount'] = ceil($totalAmount);
	      	$studentArray[$i]['discount'] = ceil($totalDiscount);
	      	$studentArray[$i]['payment'] = ceil($totalPayments);
	      	$studentArray[$i]['weaver'] = ceil($totalWeaver);
	      	$studentArray[$i]['balance'] = ceil($totalBalance);
